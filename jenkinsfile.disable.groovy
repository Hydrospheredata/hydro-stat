...
def buildDocker(){
    ...
    sh script: "docker tag hydrosphere/stat:$tagVersion hydrosphere/stat:latest", label: "Run build docker task";
}
...
def updateDockerCompose(String newVersion){
  dir('docker-compose'){
    //Change template
    sh script: "sed -i \"s/.*image:.*/    image: hydrosphere\\/stat:$newVersion/g\" ${SERVICENAME}.service.template", label: "sed ${SERVICENAME} version"
...
def updateHelmChart(String newVersion){
  dir('helm'){
    //Change template
    sh script: "sed -i \"s/.*full:.*/  full: hydrosphere\\/stat:$newVersion/g\" stat/values.yaml", label: "sed ${SERVICENAME} version"
    sh script: "sed -i \"s/.*hydrosphere\\/stat.*/    full: hydrosphere\\/stat:$newVersion/g\" dev.yaml", label: "sed ${SERVICENAME} dev stage version"

    //Refresh readme for chart
    sh script: "frigate gen stat --no-credits > stat/README.md"
      ...
      if (env.CHANGE_ID != null){
        ...
        stage('Release'){
            if (BRANCH_NAME == 'master' || BRANCH_NAME == 'main'){
                ...
                pushDocker(REGISTRYURL, "stat:latest")
