...
def kuiper_two(data1, data2):
    ...
    # print(sum(np.abs(cdfv2 - arange(len(data2)) / float(len(data2)))) / len(cdfv2) +
    #       sum(np.abs(cdfv1 - arange(len(data1)) / float(len(data1)))) / len(cdfv1))
    print(cdfv1)
    print(cdfv1.shape)
    print(cdfv2.shape)
...
def a_distance_two(data1, data2):
    ...
    cdfv1 = searchsorted(data2, data1) / float(len(data2))  # this could be more efficient
    cdfv2 = searchsorted(data1, data2) / float(len(data1))  # this could be more efficient
    return sum(np.abs(cdfv2 - arange(len(data2)) / float(len(data2)))) / len(cdfv2) + sum(
        np.abs(cdfv1 - arange(len(data1)) / float(len(data1)))) / len(cdfv1)
