...
from metric_tests.image_stats import BRISQUE
...
def get_statistic(data, stat_type, config=None, file=1):
    ...
    if stat_type == "mean":
        ...
        stat = Delaunay(data)
    elif stat_type == 'brisque':
        brisq = BRISQUE()
        # print('here')
        stat = converter(data, brisq.get_score)
        # print('here')
        plot(stat, config, file)
    elif stat_type == 'brightness':
        stat = converter(data, image_stats.brightness)
        plot(stat, config, file)
    elif stat_type == 'sharpness':
        stat = converter(data, image_stats.sharpness)
        plot(stat, config, file)
    elif stat_type == 'cpbd_metric':
        stat = converter(data, image_stats.cpbd_metric)
        plot(stat, config, file)
    elif stat_type == 'image_colorfulness':
        stat = converter(data, image_stats.image_colorfulness)
        plot(stat, config, file)
    elif stat_type == 'contrast':
        stat = converter(data, image_stats.contrast)
        plot(stat, config, file)
    elif stat_type == 'rms_contrast':
        stat = converter(data, image_stats.rms_contrast)
        plot(stat, config, file)
    elif stat_type == 'dominant_colors':
        stat = converter(data, image_stats.dominant_colors)
    return stat
